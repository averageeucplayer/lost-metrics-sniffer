...
pub async fn fake_tcp_sniffer_separate_thread() -> Result<()> {

    debug!("running example fake_tcp_sniffer_separate_thread");
...
pub async fn fake_tcp_sniffer() -> Result<()> {

    debug!("running example fake_tcp_sniffer");
    let mut service = PacketSnifferServiceWrapper::fake_tcp()?;
    let port = 6042;
    let mut rx = service.start(port)?;
